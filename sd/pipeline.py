...
def generate(prompt: str, uncond_prompt: str, input_image=None, strength=0.8, do_cfg=True, cfg_scale=7.5,
    ...
    with torch.no_grad():
        ...
        if do_cfg:
            ...
            uncond_tokens = tokenizer.batch_encode_plus([uncond_prompt], padding="max_length", max_length=77).input_idx
            uncond_tokens = torch.tensor(uncond_tokens, dtype=torch.long, device=device)
            # (Batch_Size, Seq_Len) -> (B, S, D)
        ...
        for i, timestep in enumerate(timesteps):
            ...
            if do_cfg:
                # (Batch_Size, 4, Latents_Height, Latents_Width) -> (4 * Batch_Size, 4, L_H, L_W)
